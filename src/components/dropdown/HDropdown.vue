<template>
  <h-base-field ref="fieldRef" v-bind="baseFieldProps"
      ...
      <h-base v-if="multiSelect" :class-name="inputContainerClass">
        <template v-if="((model ?? []) as string[]).length">
          <h-base class-name="flex gap-2">
            <h-button v-if="(model as string[]).length === props.items.length" :text="`${totalItems[0].label}`" post-icon-name="icons_close_close" round="circle" size="xsm"
    ...
    <template v-slot:field-additional>
      <h-base v-if="openSlot" :data-content-position="contentPosition" ref="scrollContainerRef"
...
import type { HDropdownModel, HDropdownProps } from '@/manage/types/components/dropdown'
...
const props = withDefaults(defineProps<HDropdownProps>(), {
  labelPosition: 'left',
  hasMessage: false,
  placeholder: '선택하세요',
  readonly: false,
...
const totalItems = computed(() => props.multiSelect && props.items.length > 0 ? uniqBy([totalItem, ...props.items], e => e.value) : props.items)
...
const containerHeight = computed(() => totalItems.value.length >= props.maxUnitCount ? totalItems.value.length * unitHeight.value : 0)
...
const displayedItems = ref(totalItems.value)
...
watch(totalItems, () => debouncedScrollUpdator(), { immediate: true, deep: true })
...
function updateDisplayedItems(index: number) {
  displayedItems.value = totalItems.value.slice(index, index + 5)
}

defineExpose({ getUpdatedValidState })

const fieldRef: Ref<FieldRef| null> = ref(null)
const model = defineModel<HDropdownModel>('dropdownModel')
...
const baseFieldProps = getFieldStyle<HDropdownProps>(fieldState, validState, messages, coreProps)
